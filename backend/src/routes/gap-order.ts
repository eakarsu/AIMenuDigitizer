...
// Route: /api/gap-order
// Feature: Order
// Description: Order ingestion entity (track popularity)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Order. Order ingestion entity (track popularity)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ feature: 'order', title: 'Order', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'order', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['order']);
      return res.json({ slug: 'order', items: r.rows || [] });
    }
    return res.json({ slug: 'order', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'order', items: [], error: (e && e.message) || 'Server error' });
  }
});
