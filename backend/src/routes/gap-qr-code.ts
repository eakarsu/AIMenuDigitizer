...
// Route: /api/gap-qr-code
// Feature: Qr Code
// Description: QR-code menu delivery surface
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Qr Code. QR-code menu delivery surface' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ feature: 'qr-code', title: 'Qr Code', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'qr-code', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['qr-code']);
      return res.json({ slug: 'qr-code', items: r.rows || [] });
    }
    return res.json({ slug: 'qr-code', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'qr-code', items: [], error: (e && e.message) || 'Server error' });
  }
});
