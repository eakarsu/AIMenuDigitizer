...
// Route: /api/gap-dietary-filter-recommendations
// Feature: Dietary Filter Recommendations
// Description: (dishes per dietary restriction)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Dietary Filter Recommendations. (dishes per dietary restriction)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Dietary Filter Recommendations\nDescription: (dishes per dietary restriction)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['dietary-filter-recommendations', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'dietary-filter-recommendations', title: 'Dietary Filter Recommendations', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'dietary-filter-recommendations', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['dietary-filter-recommendations']);
      return res.json({ slug: 'dietary-filter-recommendations', items: r.rows || [] });
    }
    return res.json({ slug: 'dietary-filter-recommendations', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'dietary-filter-recommendations', items: [], error: (e && e.message) || 'Server error' });
  }
});
