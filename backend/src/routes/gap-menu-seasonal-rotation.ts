...
// Route: /api/gap-menu-seasonal-rotation
// Feature: Menu Seasonal Rotation
// Description: (seasonal change recommendations)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Menu Seasonal Rotation. (seasonal change recommendations)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Menu Seasonal Rotation\nDescription: (seasonal change recommendations)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['menu-seasonal-rotation', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'menu-seasonal-rotation', title: 'Menu Seasonal Rotation', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'menu-seasonal-rotation', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['menu-seasonal-rotation']);
      return res.json({ slug: 'menu-seasonal-rotation', items: r.rows || [] });
    }
    return res.json({ slug: 'menu-seasonal-rotation', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'menu-seasonal-rotation', items: [], error: (e && e.message) || 'Server error' });
  }
});
