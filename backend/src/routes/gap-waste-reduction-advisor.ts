...
// Route: /api/gap-waste-reduction-advisor
// Feature: Waste Reduction Advisor
// Description: (link menu to inventory waste)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Waste Reduction Advisor. (link menu to inventory waste)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Waste Reduction Advisor\nDescription: (link menu to inventory waste)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['waste-reduction-advisor', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'waste-reduction-advisor', title: 'Waste Reduction Advisor', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'waste-reduction-advisor', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['waste-reduction-advisor']);
      return res.json({ slug: 'waste-reduction-advisor', items: r.rows || [] });
    }
    return res.json({ slug: 'waste-reduction-advisor', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'waste-reduction-advisor', items: [], error: (e && e.message) || 'Server error' });
  }
});
