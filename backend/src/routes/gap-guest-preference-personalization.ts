...
// Route: /api/gap-guest-preference-personalization
// Feature: Guest Preference Personalization
// Description: (per-guest dish suggestions)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Guest Preference Personalization. (per-guest dish suggestions)' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = `Feature: Guest Preference Personalization\nDescription: (per-guest dish suggestions)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['guest-preference-personalization', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'guest-preference-personalization', title: 'Guest Preference Personalization', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'guest-preference-personalization', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['guest-preference-personalization']);
      return res.json({ slug: 'guest-preference-personalization', items: r.rows || [] });
    }
    return res.json({ slug: 'guest-preference-personalization', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'guest-preference-personalization', items: [], error: (e && e.message) || 'Server error' });
  }
});
