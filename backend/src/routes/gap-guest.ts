...
// Route: /api/gap-guest
// Feature: Guest
// Description: Guest dietary preference tracking entity
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Guest. Guest dietary preference tracking entity' },
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ feature: 'guest', title: 'Guest', result, generatedAt: new Date().toISOString() });
  } catch (e: any) {
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'guest', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['guest']);
      return res.json({ slug: 'guest', items: r.rows || [] });
    }
    return res.json({ slug: 'guest', items: [] });
  } catch (e: any) {
    return res.json({ slug: 'guest', items: [], error: (e && e.message) || 'Server error' });
  }
});
